...
class UpString(underlying: String) {
  ...
  def unescapeHtml() = StringEscapeUtils.unescapeHtml4(underlying.replace("&amp;", "&"))

  def removeNewline() = underlying.replace("\n", "").replace("\r\r", "").replace("\r", "")
  
  /**
   * Converts the string to a list of tokens.
   */
  def tokens(lower: Boolean = true) = LanguageProcessing.tokens(underlying, lower)


  /**
   * Converts the string to a list of tokens, with stropwords removed.
   */
  def tokensWithoutStopwords(lower: Boolean = true) = LanguageProcessing.tokensWithoutStopwords(underlying, lower)


  /**
   * Convertes the string to a list of stemmed tokens.
   */
  def stems = LanguageProcessing.stems(underlying)


  /**
   * Convertes the string to a list of stemmed tokens, with stopwords removed.
   */
  def stemsWithoutStopwords = LanguageProcessing.stemsWithoutStopwords(underlying)
...
object UpString {
  implicit def upString(string: String) = new UpString(string)
}
...
object LanguageProcessing {
   ...
   */
  def isStopword(word: String) = Stopwords.contains(word.trim().toLowerCase())
   ...
   */
  def stemsWithoutStopwords(sentence: String) = stems(tokensWithoutStopwords(sentence))
